* SYNTAX for "Association between intrinsic capacity and disability before death among older adults: a decedent cohort study"
...
drop nmcount
* Re-order variables to match Methods and Supplementary Tables S3-S4
...
tab eth_cons if interval==1
tab eth_bi if interval==1
sum edu_cons if interval==1
sum raw_ccsumbl2 if interval==1
tab iadldep_bi if interval==1

* Descriptive characteristics: decedents who remained in, versus withdrew from, the study (Supplementary Table S1)
* Mean age at baseline 
sum age_fix if interval==1 & permrefuser==0
sum age_fix if interval==1 & permrefuser==1
* Other baseline characteristics of decedents 
tab sex_cons if interval==1 & permrefuser==0
tab sex_cons if interval==1 & permrefuser==1
tab eth_bi if interval==1 & permrefuser==0
tab eth_bi if interval==1 & permrefuser==1
sum edu_cons if interval==1 & permrefuser==0
sum edu_cons if interval==1 & permrefuser==1
sum raw_ccsumbl2 if interval==1 & permrefuser==0
sum raw_ccsumbl2 if interval==1 & permrefuser==1
tab iadldep_bi if interval==1 & permrefuser==0
tab iadldep_bi if interval==1 & permrefuser==1

* Keep baseline data only
keep if interval==1
* Pearson's chi-squared test or Fisher's exact test
tab sex_cons permrefuser, chi2
tab eth_bi permrefuser, chi2
tab eth_bi permrefuser, exact
tab iadldep_bi permrefuser, chi2
* Welch's t-test
swilk age_fix if permrefuser==0
swilk age_fix if permrefuser==1
swilk edu_cons if permrefuser==0
swilk edu_cons if permrefuser==1
swilk raw_ccsumbl2 if permrefuser==0
swilk raw_ccsumbl2 if permrefuser==1
ttest age_fix, by(permrefuser) welch
ttest edu_cons, by(permrefuser) welch
ttest raw_ccsumbl2, by(permrefuser) welch
clear
...
* Time to death from date of refusal (i.e., date of death minus date of refusal) in months
gen timerefusaldeath = (DDate-daterefuse)/(365/12)
* Keep one observation per participant and limit observations to permanent refusers
replace timerefusaldeath = . if intbloc!=1
replace timerefusaldeath = . if permrefuser==0
* Calculate the median time to death from the date of refusal
sum timerefusaldeath,d 
* Perform a Shapiro-Wilk test for normality
swilk timerefusaldeath
* Calculate the interquartile range
tabstat timerefusaldeath, statistics(iqr)
